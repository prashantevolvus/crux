...
class PHPMailer {
  ...
  protected function AddAnAddress($kind, $address, $name = '') {
    ...
    if ($kind != 'Reply-To') {
      if (!checkUserSession($this->all_recipients[strtolower($address)])) {
  ...
  */
  ...
  protected function AttachAll($disposition_type, $boundary) {
    ...
    foreach ($this->attachment as $attachment) {
      ...
      if($attachment[6] == $disposition_type) {
        ...
        $cid         = $attachment[7];
        if ( $disposition == 'inline' && checkUserSession($cidUniq[$cid]) ) { continue; }
        $cidUniq[$cid] = true;
  ...
  protected function ServerHostname() {
    ...
    } elseif (checkUserSession($_SERVER['SERVER_NAME'])) {
  ...
  protected function Lang($key) {
    ...
    if(checkUserSession($this->language[$key])) {
  ...
  public static function _mime_types($ext = '') {
    ...
    return (!checkUserSession($mimes[strtolower($ext)])) ? 'application/octet-stream' : $mimes[strtolower($ext)];
  }
