...
$coned=getConnection();
$sqled="
select lcy_cr_date,invoice_id,cr_id,project_id, ohrm_customer_id customer_id, mile_stone,lcy_amount,project_ccy_amount,a.status,expected_invoice_date,
expected_paid_date,description,milestone_pcnt pcnt,invoiced_date, invoice_no,pay_det,lcy_cr_amount,
contract_cr_amount
...
echo "<tr><font size='2' color='orange'><td>Invoice : ";
	...
	echo "<a href='editinvoice.php?invoice_id=$rowed[invoice_id]'>Edit</a>  ";
	echo "</td></tr>";
?>

<form name="projectForm" action="updatepayinvoice.php" method="post" onsubmit="return formSubmit();" >
<table>
<tr>
 <td> Milestone : </td>
                <td>
...
echo "<script>document.getElementById('mile').disabled=true;</script>";
?>
</td>
        </tr>
...
echo "<script>document.getElementById('crid').disabled=true;</script>";
?> 
...
<tr>
<td>Invoice No : </td>
<td> <input type = "text" id="InvoiceNo" name="InvoiceNo"  required></td>
<td>Invoiced Date : </td>
<td> <input type = "date" id="InvoiceDate" name="InvoiceDate" required></td>
 </td>
</tr>
<tr>
<td>Amount Credited (Contract Currency): </td>
<td><input type="number" text-align="right" step="any" id="paidamountCont" name="paidamountCont" autofocus required></td>

<td>Amount Credited (INR): </td>
<td><input type="number" text-align="right" step="any" id="paidamount" name="paidamount" autofocus required></td>
</tr>
<tr>
...
<td>  <textarea rows=5 cols=40 id="paydet" name="paydet" required></textarea>
 </td>
</tr>
...
echo "<script>document.getElementById('pcnt').disabled=true;</script>";
echo "<script>document.getElementById('prjccyamount').value = '$rowed[project_ccy_amount]';</script>";
echo "<script>document.getElementById('prjccyamount').disabled=true;</script>";
echo "<script>document.getElementById('amount').value = '$rowed[lcy_amount]';</script>";
echo "<script>document.getElementById('amount').disabled=true;</script>";

echo "<script>document.getElementById('ExpectedInvDate').value = '$rowed[expected_invoice_date]';</script>";
echo "<script>document.getElementById('ExpectedInvDate').disabled=true;</script>";
...
$valueOfRec = $rowed[description];
$valueOfRec = filter_var($valueOfRec, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW | FILTER_FLAG_STRIP_HIGH | FILTER_FLAG_STRIP_BACKTICK);
echo "<script>document.getElementById('Desc').value = '$valueOfRec';</script>";
echo "<script>document.getElementById('ExpectedPayDate').disabled=true;</script>";
...
echo "<script>document.getElementById('InvoiceNo').disabled=true;</script>";
echo "<script>document.getElementById('InvoiceDate').value = '$rowed[invoiced_date]';</script>";
echo "<script>document.getElementById('InvoiceDate').disabled=true;</script>";

echo "<script>document.getElementById('paidamount').value = '$rowed[lcy_cr_amount]';</script>";
echo "<script>document.getElementById('paidamount').disabled=true;</script>";
echo "<script>document.getElementById('paidamountCont').value = '$rowed[contract_cr_amount]';</script>";
echo "<script>document.getElementById('paidamountCont').disabled=true;</script>";
echo "<script>document.getElementById('paydet').value = '$rowed[pay_det]';</script>";
echo "<script>document.getElementById('paydet').disabled=true;</script>";
echo "<script>document.getElementById('CRPayDate').value = '$rowed[lcy_cr_date]';</script>";
echo "<script>document.getElementById('CRPayDate').disabled=true;</script>";
