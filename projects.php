...
$(document).ready(function() {
    ...
    $('#5B1').editable({
        ...
        source: ['ACTIVE', 'CLOSED', 'WARRANTY', 'DELIVERED', 'INITIATED', 'AUTHORISED', 'APPROVED',
            'DELETED', 'DEACTIVATED', 'INITIATE CLOSURE', 'AUTHORISE CLOSURE', 'PENDING INVOICE'
        ],
        validate: function(value) {
            if ($.trim(value) == '') {
                return 'This field is required';
    ...
    $('#projSearch .typeahead').typeahead(null, {
        ...
        templates: {
            ...
            suggestion: function(data) {
                return '<p>' + data.cust + ' : <strong>' + data.projname + '</strong> – ' +
...
function populateForm(projid) {
    ...
    $.getJSON(url, function(data) {
        ...
        $('#F3B2').html(amtFormat(parseFloat(data[0]['budget_initiated']) + parseFloat(data[0][
        ...
        $('#F4B3').html(amtFormat(parseFloat(data[0]['budget_initiated']) + parseFloat(data[0][
            'excess_budget_initiated'
        ])));
        ...
        if (data[0]['Contract_value'] + data[0]['cr_amt'] < 1) {
            ...
            if (data[0]['project_type_id'] == "5" || data[0]['project_type_id'] == "6") {
                $("#burn").show();
                burnRate = totalExpense * 100 / (data[0]['budget'] * data[0]['duration_so_far'] / data[0][
                    'planned_duration'
                ]);
            ...
            } else if (100 * (data[0]['received_lcy_amt'] - totalExpense) / data[0]['received_lcy_amt'] <
                10.0) {
...
function showInvoiceLine(projid) {
    ...
    $.getJSON({
        ...
        success: function(response) { //response is value returned from php
            ...
            response['data'].forEach(function(element) {
                a = {
                    x: element['INVOICE_DATE'],
                    y: parseInt(element['LCY_AMOUNT'])
                };
                data1.push(a);
                a = {
                    x: element['PAY_DATE'],
...
function showLabourLine(projid) {
    ...
    $.getJSON({
        ...
        success: function(response) { //response is value returned from php
            var i = 0;
            response['data'].forEach(function(element) {
                if (element['REPORT_MONTH'] != "TOTAL" && !isNaN(parseInt(element[
                    'UNIFIEDCOST']))) {
                    a = {
                        x: element['REPORT_MONTH'],
    ...
    $.getJSON({
        ...
        success: function(response) { //response is value returned from php
            ...
            response['data'].forEach(function(element) {
                ...
                data1.push({
                    x: 'RECEIVED',
                    y: parseInt(element['received'])
                });
                data1.push({
                    x: 'BUDGET',
                    y: parseInt(element['budget'])
                });
                data1.push({
                    x: 'COST',
                    y: parseInt(element['cost'])
                });

                data1.push({
                    x: 'BUDGET_TO_GO',
                    y: parseInt(element['budget_to_go'])
                });
                data1.push({
                    x: 'CASHFLOW',
                    y: parseInt(element['cashflow'])
                });
            ...
            <div id='row'>
                <div class="col-md-5">
                    ...
                    <div class="input-group">
                        <input id="prj" name="prj" type="input" placeholder="Enter Project Name"
                            class="form-control input-md typeahead" required="" aria-describedby="glph"
                            autocomplete="sprcatre">
                        <span class="input-group-addon" id="glph"><span
                ...
                <div class="btn-group" style="float: right">
                    <button type="button" class="btn btn-primary btn-circle btn-lg dropdown-toggle "
        ...
        <div class="tab-pane fade in active" id="GEN">
            ...
            <div class="row">
                <div class="col-sm-12 text-left">
                    ...
                    <button type="button" class="btn btn-primary" id="ACT-1" data-projectid=""
                        data-status="">ACTIVATE</button>
                    <button type="button" class="btn btn-primary" id="ACT-2" data-projectid=""
                        data-status="">CLOSED</button>
                    <button type="button" class="btn btn-primary" id="ACT-3" data-projectid="" data-status="">PENDING
                        INVOICE</button>
                </div>
            </div>
        ...
        <div class="tab-pane fade" id="FILES">
            ...
            <div class="container" id="fileUpload">
                <div class="row">
                    ...
                    <form id="uploadFilefrm" enctype="multipart/form-data" method="post" action="api/uploadFiles.php"
                        role="form">
